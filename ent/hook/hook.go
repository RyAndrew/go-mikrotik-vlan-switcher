...
// The InterfaceVlanStateFunc type is an adapter to allow the use of ordinary
// function as InterfaceVlanState mutator.
type InterfaceVlanStateFunc func(context.Context, *ent.InterfaceVlanStateMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f InterfaceVlanStateFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.InterfaceVlanStateMutation); ok {
		return f(ctx, mv)
	}
